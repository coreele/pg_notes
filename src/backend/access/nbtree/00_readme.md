...
VACUUM can ever mark pointed-to items LP*UNUSED in the heap, and since
this only ever happens \_after* btbulkdelete returns, having index scans
...
their LP*DEAD bit set when every table TID within the posting list is
...
that \_some* deletion operation that targets related nearby-in-table TIDs
...
_between_ items on the page to be split, at least if you pretend that the
