...
1. **Buffer Manager**：管**磁盘数据**。核心是“缓存数据”，例如用 LRU 等算法把热点页留在内存，解决 IO 慢的问题。
1. **ResourceOwner**：管**资源归属**。核心是“谁申请、谁负责释放”，用于跟踪事务/子事务持有的资源（如 buffer pin、锁、临时文件等），在事务结束或出错时**统一回收**，保证不会泄漏。
