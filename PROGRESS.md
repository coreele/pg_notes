...
| 模块 | 整体 | 一句话 |
|------|------|--------|
| **wal** | **掌握（核心三课）** | Record / FPW / LSN 已闭环；缺 MTR、流复制 |
...
**当前主线建议**：先收尾 WAL（MTR → 流复制），再补 storage 空洞与 transaction 锁/CLOG，最后才进 optimizer。
...
| What & Why: Mini-Transaction | — | **未开始** | — | **下一课**：多页原子、critical section、B-tree split 与 WAL |
...
| 优先级 | 主题 | 原因 |
|--------|------|------|
| **P0** | Mini-Transaction | 接 LSN/FPW；打通 nbtree 多页修改与 WAL |
...
```text
阶段 0  已做：INSERT 链路 · WAL Record · FPW · LSN
阶段 1  WAL 收尾：MTR → 流复制/Slot →（可选）Logical Decoding
...
4. （下一课）一次涉及多页的修改，为何需要 MTR / critical section  
